...
import { whatsappChipState } from '@/ui/lib/whatsapp-chip';

export function WhatsAppStatusChip() {
  const { connected, pushname, catalogCount, connectedCount } = useWhatsAppStatus();
  if (connected === null) {
    return null;
  }

  const chip = whatsappChipState({
    catalogCount,
    connectedCount,
  ...
  return (
    <Link
      ...
      title={pushname ? `Conectado como ${pushname}` : chip.label}
    >
      {chip.tone === 'ok' ? (
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm">
      <p className="text-foreground">
        Nenhuma linha do WhatsApp está conectada. Escaneie o QR para receber conversas.
      </p>
