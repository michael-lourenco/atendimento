...
3. Rodar migrations `001`–`021` **nessa ordem** nesse projeto
...
Migrations em `infra/supabase/migrations/` (SQL versionado). Sem editar schema só no dashboard. `002_flow_session_paused.sql` adiciona `flow_sessions.paused`. `003_sales_intake_seed.sql` grava setores Comercial/Demonstração/Cliente, agentes, etiquetas e o fluxo `inicio` de triagem. `004_operator_agent.sql` faz o perfil nascer como agente e o primeiro usuário ser admin. `005_whatsapp_lines.sql` adiciona `whatsapp_numbers.instance_name` e `conversations.whatsapp_number_id`. `006_conversation_thread.sql` garante essas colunas (`if not exists`, caso a 005 não tenha rodado) e cria índice único `(contact_phone, whatsapp_number_id)` **onde** `whatsapp_number_id` não é null — uma thread por contato+linha. `flow_sessions.contact_id` guarda o id da thread, não o telefone isolado quando a conversa é composta. `007_quick_replies.sql` cria `quick_replies` (`id` text PK, `title`, `body`, `created_at`). RLS igual às outras tabelas de catálogo (`authenticated` all: `using (true) with check (true)`). Sem `company_id`. Sem seed de frases (prod começa vazio; 2–3 exemplos só no mock de dev/test). `008_miss_returns_to_menu.sql` liga o passo `miss` do fluxo `inicio` a `menu`, para reapresentar as opções sem o “Olá”. `009_welcome_michael.sql` troca a saudação do fluxo `inicio` de Atimo para Michael. `010_schedule_conversation.sql` adiciona `scheduled_messages.conversation_id` (text, nullable) — thread da linha quando o agendamento nasce no chat. Sem essa coluna, o save **omite** o campo (PGRST204). `011_conversation_last_message.sql` adiciona `conversations.last_message` (jsonb, snapshot da última mensagem para a prévia da inbox). Sem essa coluna, upsert **não** envia `last_message` (PGRST204); a inbox hidrata a prévia em memória. `012_unique_agent_email.sql` colapsa `agents` duplicados pelo mesmo e-mail (mantém a linha cujo `id` = `profiles.id`; senão a mais antiga), remapeia `conversations.assigned_agent_id`, e cria índice único em `lower(trim(email))`. O trigger `handle_new_user` deixa de inserir agente se o e-mail já existir. `013_contact_avatar.sql` adiciona `contacts.avatar_url` e `conversations.contact_avatar_url` (text, nullable). Sem essas colunas, o save **omite** os campos (PGRST204). `014_flow_option_number_hint.sql` atualiza o texto do passo `miss` do fluxo `inicio` para aceitar o **número** da opção além do texto. `015_flow_delete_cascade.sql` faz `flow_sessions.flow_id` **ON DELETE CASCADE** e `chatbots.flow_id` **ON DELETE SET NULL**, para excluir um fluxo que já teve conversa. `016_split_intake_flows.sql` parte o seed de triagem: `inicio` fica o menu; cria/atualiza `sistema`, `demo`, `cliente` e `comercial` (ativos, ligados por `goToFlow`). `017_flow_editor_session.sql` adiciona `flows.keywords` (jsonb `[]`) e `flow_sessions.return_stack` (jsonb `[]`). Sem essas colunas, o save **omite** os campos (PGRST204) para o roteiro ainda gravar; palavras-chave e pilha de retorno só persistem depois da migration. `018_intake_sales_copy.sql` atualiza os textos de `inicio`, `sistema`, `demo`, `cliente` e `comercial`. `019_inbox_hours_queue.sql` adiciona `conversations.assigned_at`, `chatbots.business_hours` (jsonb), `flow_sessions.outside_hours_notified`, delay no `welcome`/`menu` do `inicio`, e publica `conversations`/`messages` no Realtime. Sem essas colunas, o save **omite** os campos (PGRST204). `020_message_reactions.sql` adiciona `messages.reactions` (jsonb `[]`). Sem a coluna, o save **omite** o campo (PGRST204). `021_quote_typing.sql` adiciona `messages.quoted_message_id`, `quoted_content`, `quoted_from` (text) e `conversations.contact_typing_at` (timestamptz). Sem essas colunas, o save **omite** os campos (PGRST204). `022_quick_reply_audio.sql` adiciona `quick_replies.media_kind` (text, nullable; `audio` \| `image` \| `video` \| `document`). Sem a coluna, o save **omite** o campo (PGRST204). Sem migration nova: a coluna já é texto livre. `023_quick_reply_department.sql` adiciona `quick_replies.department_id` (text, nullable). Sem a coluna, o save **omite** o campo (PGRST204). `024_bot_behavior.sql` adiciona `chatbots.behavior` (jsonb). Sem a coluna, o save **omite** o campo (PGRST204). `025_line_bot_behavior.sql` adiciona `whatsapp_numbers.behavior` (jsonb). Sem a coluna, o save **omite** o campo (PGRST204). `026_line_entry_flow.sql` adiciona `whatsapp_numbers.flow_id` (text, FK para `flows` ON DELETE SET NULL). Sem a coluna, o save **omite** o campo (PGRST204). `027_line_business_hours.sql` adiciona `whatsapp_numbers.business_hours` (jsonb). Sem a coluna, o save **omite** o campo (PGRST204). `028_conversation_viewer.sql` adiciona `conversations.viewer_agent_id`, `viewer_agent_name` (text) e `viewer_at` (timestamptz). Sem essas colunas, o save **omite** os campos (PGRST204). `029_flow_publish_and_session_turn.sql` adiciona `flows.published_steps` (jsonb) e em `flow_sessions`: `consumed_incoming_at`, `miss_streak`, `media_hint_step_id`. Sem essas colunas, o save **omite** os campos (PGRST204). Admin vê o que falta em `GET /api/admin/schema-health`.

## Storage

Bucket privado `media` para anexos WhatsApp na v1. Objetos em `messages/{id}` (mídia da mensagem), `contacts/{id}` (foto de perfil), `quick-replies/{id}` (áudio, imagem, vídeo ou PDF de resposta rápida) e `flows/{flowId}/{stepId}` (imagem, áudio, vídeo ou PDF do passo `message`). Sem coluna nova: `mediaUrl`/`mediaKind` já vivem em `Flow.steps` (JSONB). Sem bucket novo. O painel não usa URL pública: `GET /api/messages/{id}/media`, `GET /api/contacts/{id}/avatar`, `GET /api/quick-replies/{id}/media` e `GET /api/flows/{flowId}/steps/{stepId}/media` leem o Storage (sessão do operador; bytes via service_role no servidor). O motor (`loadFlowStepMedia`) lê o path `flows/{flowId}/{stepId}` com `service_role` para enviar no WhatsApp; URL `http(s)` pública no passo continua válida (legado, sem passar pelo bucket). Sem AWS S3 nesta fase.

## Composição

- Implementações em `infra/supabase/` (`SupabaseFlowRepository`, etc.) cumprindo as portas de `core/repositories`.
- Composition root na borda (`app/` + `ServiceLocator` / `serverLocator` / `clientUseCases`): **prod/dev com env Supabase** usa repos reais; **test** continua fake/mock.
- Use cases **não** importam o locator. Injeção no construtor, na borda.
