...
**Fluxos:** `/dashboard/flows` é a **lista**. Criar/editar abre **tela cheia** (`/dashboard/flows/new` ou `/dashboard/flows/[flowId]`). O **quadro** (`@xyflow/react`) é a área principal (arrastar, zoom, ligar bolinhas). O formulário do passo fica num **painel sobre o quadro** (não empilha cartões no lugar do canvas). Paleta de blocos em destaque (mesma borda colorida dos nós do quadro; arrastar ou clicar). Paleta: Mensagem, Pergunta, Definir setor, Ir para fluxo, **Passar para atendente**. Simulador recolhido no painel. Avisos de roteiro quebrado. Alterações não salvas: banner + `Ctrl`/`⌘`+S. Duplicar fluxo na lista e bloco no quadro. **Ir para fluxo:** botão abre o destino (`?from=`). `goToFlow` com “Ao voltar” empilha retorno. `handoff` pausa para o time. Lista: selo WhatsApp; **Excluir** apaga sessões.
