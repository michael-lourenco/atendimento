...
**Respostas rápidas:** `/dashboard/quick-replies` é **funcional**. Item no grupo **Atendimento**, **sem** `adminOnly`: atendente e admin veem. `isAdminPath` **não** inclui essa rota (atendente que abre a URL permanece nela). Página: lista `title` + prévia do `body` **ou** selo **Áudio** + player; criar/editar (`title` + texto/`body` opcional + áudio: upload **ou** PTT no formulário, mesmo `usePttRecorder` do chat; soltar anexa, não envia WhatsApp; máx. 16 MB); excluir com diálogo Confirmar/Cancelar; empty state com “Nova resposta”. Falha ao listar/salvar/excluir (ex. tabela ausente, `PGRST205`) aparece na página. Catálogo da empresa (`QuickReplyCatalogUseCase` / `IQuickReplyRepository`). Título/texto no client (como Etiquetas). Áudio: `PUT`/`GET`/`DELETE /api/quick-replies/{id}/media`. Precisa das migrations `007_quick_replies.sql` e `022_quick_reply_audio.sql`.
