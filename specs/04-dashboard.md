...
**Fluxos:** o formulário edita o roteiro em linguagem de atendimento. Blocos visíveis: **Mensagem**, **Pergunta**, **Definir setor** (e condição só se não pertencer a uma pergunta). Cada bloco começa **recolhido** (clique para editar); o recém-adicionado abre sozinho. **Adicionar** escolhe o tipo (Mensagem, Pergunta, Definir setor ou **Ir para fluxo**) e anexa no **fim**, com rolagem até o bloco. O `id` do passo e o `nextStepId` **não** são digitados: o próximo passo (e os ramos “se sim / se não”) são escolhidos na lista. Condições da cadeia de uma **Pergunta** ficam **dentro dela** (“Como o bot reconhece as respostas”), não como cartões soltos. Na Pergunta, cada opção é uma **linha** (não lista separada por vírgula), com o destino em texto (“Vai para: …”) — setor, mensagem, encerrar ou **outro fluxo ativo**. “Aplicar opções no roteiro” gera a cadeia se ainda faltar. Destinos da Pergunta só são recalculados quando o texto das opções (ou o catálogo de setores) muda — não a cada render. Na **Condição**, as opções da pergunta aparecem como botões; clicar grava o texto e `equals`. Prévia **Como o cliente vê** fica **acima** dos blocos; o mapa de ligações fica recolhido. `action` + setor = `setDepartment`. `action` + fluxo = `goToFlow` (a sessão muda para o destino no mesmo turno; novos contatos continuam no selo WhatsApp). Sem volta automática ao fluxo de origem. Opções no WhatsApp saem numeradas (`1. …`); o cliente pode responder com o número ou o texto. Na lista, o fluxo que `resolveActiveFlow` escolher ganha o selo WhatsApp. Empty state com ação de criar. **Excluir** apaga o fluxo e as sessões daquele roteiro (a próxima mensagem do contato cai no fluxo de entrada); chatbots que apontavam para ele ficam sem `flowId`. Se a exclusão falhar, a página mostra o erro (não só o console).
