...
- Layout dashboard: sidebar **expansível** (nomes visíveis; pode recolher para ícones) + header com marca, **tela atual**, selo de conexão WhatsApp + tema claro/escuro. **Tema claro e escuro** próximos do WhatsApp Web (tokens em `app/globals.css`): verde `#00A884`; claro = fundo `#F0F2F5`, chat `#EFEAE2`, bolha outgoing `#D9FDD3`; escuro = fundo `#111B21`, chat `#0B141A`, bolha outgoing `#005C4B`.
...
- KPIs: `GetDashboardMetricsUseCase` calcula totais, conversas ativas, taxa de resposta, **volume por setor** e **tempo médio até Assumir** (`assignedAt` − `createdAt`). Sem percentuais inventados; média omitida se ninguém assumiu.
